...
<template>
  <div v-if="isConnected">
      <div v-if="dtcCodes.length > 0" class="space-y-4">
        <div class="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border border-red-200 dark:border-red-800">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-bold text-red-600 dark:text-red-400">Diagnostic Trouble Codes</h3>
            <UButton 
              color="error" 
              variant="soft" 
              label="Clear Codes" 
        ...
        <UModal v-model="showClearModal">
          <div class="p-6 space-y-4">
            <h3 class="text-lg font-bold">Clear Diagnostic Codes?</h3>
            <p class="text-sm text-dimmed">This will reset the check engine light. Please provide a Work Order reference for this action.</p>
      ...
      <div v-else class="text-center p-4 text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-200 dark:border-green-800">
        ...
        <p class="font-medium">No Fault Codes Detected</p>
      </div>
  </div>
</template>
