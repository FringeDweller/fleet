...
const { data: forms, refresh: refreshForms } = await useFetch<any[]>('/api/forms/context', {
    query: {
        module: props.module,
...
<template>
  <div class="space-y-8">
    ...
    <div v-if="forms && forms.length > 0" class="space-y-4">
        <h3 class="text-sm font-bold uppercase tracking-wider text-dimmed px-1">Available Forms</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="form in forms" :key="form.id" class="border rounded-lg bg-white dark:bg-gray-900 overflow-hidden flex flex-col shadow-sm">
                <div class="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-b flex justify-between items-center">
                    ...
                    <UButton label="Fill" size="xs" variant="soft" icon="i-lucide-edit-3" @click="openForm(form)" />
                </div>
                <div class="p-4 flex-1">
                    <p v-if="form.description" class="text-xs text-dimmed line-clamp-2">{{ form.description }}</p>
                    <p v-else class="text-xs text-dimmed italic">No description provided.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div v-if="submissions && submissions.length > 0" class="space-y-4">
        <h3 class="text-sm font-bold uppercase tracking-wider text-dimmed px-1">Previous Submissions</h3>
        <div class="border rounded-lg divide-y bg-white dark:bg-gray-900 shadow-sm">
            <div v-for="sub in submissions" :key="sub.id" class="p-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer group" @click="viewSubmission(sub)">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-primary-50 dark:bg-primary-900/20 rounded-full text-primary-500">
                        <UIcon name="i-lucide-file-text" class="w-4 h-4" />
                    </div>
                    <div>
                        <div class="font-medium text-sm">Submission on {{ new Date(sub.createdAt).toLocaleString() }}</div>
                        <div class="text-xs text-dimmed flex gap-2">
                            <span>By User {{ sub.submittedBy.slice(0,8) }}</span>
                        </div>
                    </div>
                </div>
                <UButton icon="i-lucide-eye" variant="ghost" size="xs" color="neutral" class="opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
        </div>
    </div>
    ...
    <UModal v-model="showFormModal">
        <div class="p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-6 sticky top-0 bg-white dark:bg-gray-900 z-10">
                <h3 class="text-xl font-bold">{{ selectedForm?.title }}</h3>
                <UButton icon="i-lucide-x" variant="ghost" color="neutral" @click="showFormModal = false" />
            </div>
    ...
    <UModal v-model="showSubmissionModal">
        <div class="p-6">
            <div class="flex justify-between items-center border-b pb-4 mb-6 sticky top-0 bg-white dark:bg-gray-900 z-10">
                <h3 class="text-xl font-bold">Form Submission</h3>
                <UButton icon="i-lucide-x" variant="ghost" color="neutral" @click="showSubmissionModal = false" />
            </div>
            ...
            <div v-if="selectedSubmission" class="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
                ...
                <div class="space-y-4">
                    <div v-for="(val, key) in selectedSubmission.data" :key="key" class="border-b border-default pb-3">
                        <div class="text-xs text-dimmed font-bold uppercase mb-1">{{ key }}</div>
                        <div class="text-sm whitespace-pre-wrap">{{ val }}</div>
                    </div>
                </div>
            </div>
