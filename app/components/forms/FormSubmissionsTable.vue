...
const columns = computed(() => {
    ...
    if (props.schema) {
        props.schema.slice(0, 3).forEach((f: any) => {
            if (f.type !== 'section') {
...
<template>
  <div class="space-y-4">
    <div class="flex justify-between items-center">
        ...
        <UButton icon="i-lucide-download" label="Export CSV" variant="soft" @click="exportToCsv" />
    </div>
    ...
    <UModal v-model="showModal">
        <div class="p-6 space-y-6">
            <div class="flex justify-between items-center border-b pb-4 mb-4 sticky top-0 bg-white dark:bg-gray-900 z-10">
                <h3 class="text-xl font-bold">Submission Detail</h3>
            ...
            <div v-if="selectedSubmission" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                <div v-for="field in (schema as any[])" :key="field.id">
