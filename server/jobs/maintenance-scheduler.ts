...
async function checkAndCreateWorkOrder(
  schedule: typeof maintenanceSchedules.$inferSelect,
  asset: { id: string; currentMileage: string | null; currentHours: string | null }
) {
  // Check active WO
  const openWo = await db
    .select()
    .from(workOrders)
    .where(
      and(
...
async function createWorkOrder(
  ...
  // Add parts
  if (schedule.taskId) {
    const requiredParts = await db
      .select()
      .from(taskParts)
      .where(eq(taskParts.taskId, schedule.taskId))
