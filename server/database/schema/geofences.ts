...
export const geofences = pgTable('geofences', {
  ...
  type: text('type', { enum: ['circle', 'polygon'] })
    .notNull()
    .default('circle'),
  // For circle
  ...
  category: text('category', { enum: ['depot', 'job_site', 'restricted', 'other'] })
    .notNull()
    .default('depot'),
  activeHours: jsonb('active_hours'), // { start: '08:00', end: '17:00', days: [1,2,3,4,5] }

  alertOnEntry: text('alert_on_entry', { enum: ['always', 'never', 'after_hours'] })
    .notNull()
    .default('always'),
  alertOnExit: text('alert_on_exit', { enum: ['always', 'never', 'after_hours'] })
    .notNull()
    .default('always'),
