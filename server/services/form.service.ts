...
export const formService = {
  ...
  async getForm(id: string, organizationId: string) {
    const result = await db
      .select()
      .from(customForms)
      .where(and(eq(customForms.id, id), eq(customForms.organizationId, organizationId)))
      .limit(1)
  ...
  async createForm(data: typeof customForms.$inferInsert) {
    ...
    if (!form.formGroupId) {
      await db.update(customForms).set({ formGroupId: form.id }).where(eq(customForms.id, form.id))
  ...
  async updateForm(
    ...
    data: Partial<typeof customForms.$inferInsert>
  ) {
    ...
    if (existing.status === 'published' && (data.schema || data.title)) {
      ...
      if (draft[0]) {
        ...
        const maxVersionResult = await db
          .select({ maxV: sql<number>`max(${customForms.version})` })
          .from(customForms)
  ...
  async createAssignment(data: typeof formAssignments.$inferInsert) {
    const [assignment] = await db.insert(formAssignments).values(data).returning()
    return assignment
  },
  ...
  async deleteAssignment(id: string, organizationId: string) {
    await db
      ...
      .where(and(eq(formAssignments.id, id), eq(formAssignments.organizationId, organizationId)))
  },

  async getFormsForContext(
    organizationId: string,
    module: string,
