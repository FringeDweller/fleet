...
import { geofenceEvents, geofences, notifications, users } from '../database/schema'
...
export const geofenceService = {
  ...
  async updateGeofence(
    ...
    data: Partial<typeof geofences.$inferInsert>
  ) {
    const [geofence] = await db
      .update(geofences)
      .set({ ...data, updatedAt: new Date() })
      .where(and(eq(geofences.id, id), eq(geofences.organizationId, organizationId)))
      .returning()
    return geofence
  },
  ...
  async processLocation(
    ...
    for (const gf of allGeofences) {
      ...
      const [lastEvent] = await db
        .select()
        .from(geofenceEvents)
        .where(and(eq(geofenceEvents.assetId, assetId), eq(geofenceEvents.geofenceId, gf.id)))
      ...
      if (isIn && !wasIn) {
        ...
        const shouldAlert =
          gf.alertOnExit === 'always' || (gf.alertOnExit === 'after_hours' && this.isAfterHours(gf))
  ...
  isAfterHours(geofence: typeof geofences.$inferSelect) {
    ...
    const { start, end, days } = geofence.activeHours as {
      start: string
      end: string
      days: number[]
    }
  ...
  async triggerAlert(
    assetLabel: string,
    geofence: typeof geofences.$inferSelect,
    type: string,
    organizationId: string
  ) {
    let title = ''
    ...
    const usersToNotify = await db
      ...
      .from(users)
      .where(
        and(eq(users.organizationId, organizationId), inArray(users.role, ['admin', 'manager']))
      )
  ...
  async getJobSiteLogs(
    organizationId: string,
    options: { assetId?: string; geofenceId?: string } = {}
  ) {
    ...
    if (options.geofenceId) {
      ...
      const jobSites = await db.query.geofences.findMany({
        where: and(eq(geofences.organizationId, organizationId), eq(geofences.category, 'job_site'))
      })
      ...
      if (event.type === 'entry') {
        ...
        logs.push({
          ...
          durationMinutes: Math.round(
            (event.createdAt.getTime() - entry.createdAt.getTime()) / 60000
          )
        })
